...
def old_qa_glossary_propagation_of_sap_hana():
    ...
    #output_file_path = str(start) + "_to_" + str(end) + "_qa_sap_hana_glossary_propagation_results"
    # NOTE: HARDCODING
    output_file_path = "451_to_500_qa_sap_hana_glossary_propagation_results.txt"
   
    directory = "glossary_propagation_outputs/sap_hana_outputs/qa"
    os.makedirs(directory, exist_ok=True) # Create the directory if it doesn't exist
    output_file_path = os.path.join(directory, output_file_path)

    sap_hana_view_details = qa_pulled_entities.get("data_sources").get("sap_hana").get("sap_hana_view").get("all_entity_details")
    with open(output_file_path, 'w') as file:
        file.flush()
        file_and_dicts = prepare_for_propagation_of_sap_hana(client, sap_hana_view_details, file, start, end)
        ...
        file = apply_glossary_terms_and_write_output_of_sap_hana(client, file, updated_dict_for_string_matches,updated_dict_for_guids_of_a_glossary_term)
        file.flush()
        file.close()
        print("Propagation across SAP HANA views in QA is complete\n\n")


def old_prod_glossary_propagation_of_sap_hana():
    ...
    output_file_path = str(start) + "_to_" + str(end) + "_prod_sap_hana_glossary_propagation_results.txt"
    # NOTE: HARDCODING
    #output_file_path = "451_to_500_qa_sap_hana_glossary_propagation_results.txt"
   
    directory = "glossary_propagation_outputs/sap_hana_outputs/prod"
    os.makedirs(directory, exist_ok=True) # Create the directory if it doesn't exist
    output_file_path = os.path.join(directory, output_file_path)
    ...
    with open(output_file_path, 'w') as file:
        ...
        file.flush()
        file.close()
        print("Propagation across SAP HANA views in Prod is complete\n\n")
