...
def salsify_lineage_test(client, file_name):
    """
    ...
    #initialize previous record name to keep track of changing record name
    ...
    for sheet_name in xls.sheet_names:
        ...
        for index, row in df.iterrows():
            record_name=str(row["Record name"])

            #if the record name is changed, again fetch the guid and create qualified record name
            if record_name!=previous_record_name:
                record_qualified_name = "salsify://id//record/" + record_name 
                guid_counter = -1002
...
def main():
    ...
    file_name='salsify_final_mappings_test.xlsx'
    salsify_lineage_test(qa_client, file_name)
